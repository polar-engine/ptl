...
namespace ptl {
	class random {
	  ...
	  public:
		random(uint64_t seed = 0) { engine.seed(seed); }

		inline explicit operator uint64_t() { return engine(); }
		...
		inline explicit operator decimal() { return decimal(double(*this)); }
